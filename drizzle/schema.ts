...
export const notificationConfigs = mysqlTable("notification_configs", {
  ...
  userId: int("userId").notNull(),
  locationId: int("locationId").notNull(),
  // 通知类型：dingtalk（钉钉群机器人）、pushdeer（PushDeer推送）
  type: mysqlEnum("type", ["dingtalk", "pushdeer"]).notNull(),
  // 通知渠道的标识：钉钉为webhook URL，PushDeer为pushkey
  channelId: varchar("channelId", { length: 500 }).notNull(),
  // 通知渠道的显示名称（如"团队群"、"个人推送"）
  name: varchar("name", { length: 255 }).notNull(),
  // 晨雾概率阈值（%），超过此阈值时发送通知，默认80
  threshold: int("threshold").default(80).notNull(),
  // 是否启用此通知配置
...
export const notificationHistory = mysqlTable("notification_history", {
  ...
  userId: int("userId").notNull(),
  locationId: int("locationId").notNull(),
  configId: int("configId").notNull(), // 关联的通知配置
  // 通知类型
  type: mysqlEnum("type", ["dingtalk", "pushdeer"]).notNull(),
  // 触发通知的晨雾概率（%）
  fogProbability: int("fogProbability").notNull(),
  // 通知内容
  message: text("message").notNull(),
  // 发送状态：success（成功）、failed（失败）
  status: mysqlEnum("status", ["success", "failed"]).notNull(),
  // 失败原因（如果status为failed）
